...
def bag_of_words_encoding(stem_words, pattern_word_tags_list):
    ...
    for word_tags in pattern_word_tags_list:
        ...
        '''
    for i in pattern_word_tags_list:
        bag_of_words = []
        pattern_words = i[0]
        for j in pattern_words:
             index = pattern_words.index(j)
             j = stemmer.stem(j.lower())
             pattern_words[index] = j
        for k in stem_words:
            if k in pattern_words:
...
def preprocess_train_data():
    ...
    def preprocessing_data(training_data):
    ...
    train_x, train_y = preprocessing_data(training_data)
